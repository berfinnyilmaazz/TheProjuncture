...
export const postTaskActivity = async (req, res) => {
    try {
      ...
      return res.status(200).json({ status: false, message: "Activity posted successfully" });
...
export const dashboardStatistics = async (req, res) => {
    try {
        ...
        const users = await User.find({isActive: true})
            .select("name title role isAdmin creadtedAt")
            .limit(10)
            .sort({ _id: -1 });
...
export const createSubTask = async (req, res) => {
    try {
        ...
        console.log("Subtask POST body:", req.body);
         ...
         res
         .status(200)
         .json({ status: false, message: "SubTask added successfully."});
...
export const updateTask = async (req, res) => {
    try {
        ...
        res
            .status(200)
            .json({ status: false, message: "Task duplicated successfully."});
    } catch (error) {
        console.log(error);
        return res.status(400).json({ status: false, message: error.message});
    }
